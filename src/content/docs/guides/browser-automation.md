...
dassi can grab an image directly from a web page and carry it into the current session — no downloading or manual steps required. Once captured, it can attach that image to a file input on any other tab.

A typical example: copying a product photo from a catalog page and uploading it into a listing form on another site. dassi handles the fetch using the page's own cookies and session, so images behind a login work just as well as public ones.

This applies to standard `<img>` elements on the page. For canvas graphics, charts, or images generated by the page itself, dassi uses a screenshot instead.

<!-- TODO: screenshot of image capture in action -->
